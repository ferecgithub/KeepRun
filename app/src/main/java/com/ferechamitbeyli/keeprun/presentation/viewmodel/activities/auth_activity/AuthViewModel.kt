package com.ferechamitbeyli.keeprun.presentation.viewmodel.activities.auth_activity
...
class AuthViewModel @Inject constructor(
    ...
    private val _authEventsChannel = Channel<AuthEvents>()
    val authEventsFlow = _authEventsChannel.receiveAsFlow()
    ...
    fun validateBeforeSignIn(email: String, password: String) = viewModelScope.launch {
        ...
        when {
            (emailValidation != ValidationResults.SUCCESS) -> {
                _authEventsChannel.send(AuthEvents.ErrorCode(emailValidation))
            }
            (passwordValidation != ValidationResults.SUCCESS) -> {
                _authEventsChannel.send(AuthEvents.ErrorCode(passwordValidation))
            }
            else -> {
        ...
        viewModelScope.launch {
            ...
            when {
                ...
                (passwordsValidation != ValidationResults.SUCCESS) -> {
                    _authEventsChannel.send(AuthEvents.ErrorCode(passwordsValidation))
                }
                else -> {
    ...
    private fun signInUser(email: String, password: String) = viewModelScope.launch {
        try {
            ...
            user?.let {
                ...
                _authEventsChannel.send(AuthEvents.Message("Login Success"))
            }
        } catch (e: Exception) {
            val error = e.toString().split(":").toTypedArray()
            Log.d(TAG, "signInUser: ${error[1]}")
            _authEventsChannel.send(AuthEvents.Error(error[1]))
        }
    }

    private fun signUpUser(email: String, password: String, username: String) = viewModelScope.launch {
        try {
            val user = authRepository.signUp(email, password, username)
            user?.let {
                _firebaseUser.postValue(it)
                _authEventsChannel.send(AuthEvents.Message("Sign Up Success"))
            }
        } catch (e: Exception) {
            val error = e.toString().split(":").toTypedArray()
            Log.d(TAG, "signInUser: ${error[1]}")
    ...
    fun signOut() = viewModelScope.launch {
        try {
            ...
            user?.let {
                _authEventsChannel.send(AuthEvents.Message("Logout Failure"))
            } ?: _authEventsChannel.send(AuthEvents.Message("Sign Out Successful"))
    ...
    private fun sendPasswordResetEmail(email: String) = viewModelScope.launch {
        try {
            ...
            if (result) {
                _authEventsChannel.send(AuthEvents.Message("reset email sent"))
            } else {
                _authEventsChannel.send(AuthEvents.Error("could not send password reset"))
            }
        } catch (e: Exception) {
            val error = e.toString().split(":").toTypedArray()
            Log.d(TAG, "signInUser: ${error[1]}")
    ...
    sealed class AuthEvents {
        ...
        data class ErrorCode(val result: ValidationResults) : AuthEvents()
